...
export default function TabelaMensagens(props) {
    ...
    function excluirMensagemFrontEnd(mensagem){
        const atualData = new Date().toLocaleString("pt-BR");
        const diferencaData = Math.floor((atualData - mensagem.dataHora)/6000);
        if(diferencaData<=5)
        {
            if(window.confirm("Deseja realmente excluir a mensagem " + mensagem.mensagem)){
                despachante(apagarMensagem(mensagem)); //envia a action apagarProduto
            }
        }
        else{
            <Alert variant="danger">Tempo de exclusao excedido</Alert>
        }
    }
    ...
    if (estado === ESTADO.PENDENTE){
        ...
        return (
            <>
                <Container>
                    <p>Usuario logado: {props.usuario.usuario}</p>
